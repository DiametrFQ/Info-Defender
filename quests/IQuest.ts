export default interface IQuest {
  ...
  answers: answer[];
  generalAnswerEvent?: answerEvent[];
}
interface description {
  backCoord: [number, number];
  coord: [number, number]; // [760, 500]
  text: string;
...
interface answerEvent {
  name?: string;
  setBackDescriptionDisplay?: [number, number];
  deleteAnswers?: number[];
  setQuestText?: string;
  numberSetAnswer?: number;
  setAnswerText?: string;
  setAnswerEvent?: answerEvent | string;
  questDestroy?: boolean;
  createBubbles?: Buble[];
}
interface Buble {
  coord: [number, number];
  nameImg: string;
  eventAfterDeath?: eventAfterDeathBuble;
}
interface eventAfterDeathBuble {
  create?: {
    description: description;
    answer: answer;
  };
  setDescriptionCoord?: [number, number];
...
//   constructor(
//     private _coord: [760, 500],
//     private _scene: Game,
//     readonly _text: `Сотрудник:"С приходом электриков,\n у меня компьютер стал жить своей жизнью"`,
//     readonly _textDescription: "asdasd"
//   ) {
